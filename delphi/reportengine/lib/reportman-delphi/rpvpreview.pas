...
uses
  ...
  VCL.StdCtrls,rpmetafile, VCL.ComCtrls,rphtmldriver,rppreviewcontrol,
  rpgdidriver, VCL.ExtCtrls,VCL.Menus,rptypes,rpexceldriver,rptextdriver,rpsvgdriver,
  rpcsvdriver,rpgraphutilsvcl,rppreviewmeta,rpbasereport,rpreport,rppagesetupvcl,
  VCL.ActnList, VCL.ImgList,VCL.Printers,rpmdconsts, VCL.ToolWin, VCL.Mask, rpmaskedit,rpmunits,
...
type
  ...
  private
    ...
    procedure FindNext;
    procedure SetTouchEnabled(newvalue:boolean);
  public
    { Public declarations }
    property PreviewControl:TRpPreviewmeta read fpreviewcontrol write SetPreviewControl;
    property TouchEnabled:Boolean read FTouchEnabled write SetTouchEnabled;
  end;
...
procedure TFRpVPreview.SetTouchEnabled(newvalue: Boolean);
begin
 if (newvalue = FTouchEnabled) then
  exit;
 FTouchEnabled := newvalue;
 if (newvalue) then
 begin
  btoolbar.ButtonHeight := Round(oldbuttonheight*1.5);
  btoolbar.ButtonWidth := Round(oldbuttonwidth*1.5);
 end
 else
 begin
  btoolbar.ButtonHeight := oldbuttonheight;
  btoolbar.ButtonWidth := oldbuttonwidth;
 end;
end;
