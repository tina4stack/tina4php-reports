...
{$IFDEF FPC}
  MaskEdit,
{$ELSE}
  Windows, Messages,VCL.Mask,
{$ENDIF}
  VCL.StdCtrls, rpmdconsts;
...
begin
 ...
 case FEditType of
  ...
  tedatetime:
   begin
{$IFDEF DELPHI2009UP}
    FValidchars:=[FormatSettings.DateSeparator,FormatSettings.Timeseparator,'0'..'9'];
{$ELSE}
...
{$ENDIF}
  ...
  tedate:
   begin
{$IFDEF DELPHI2009UP}
    FValidchars:=[FormatSettings.DateSeparator,'0'..'9'];
{$ELSE}
...
{$ENDIF}
  ...
  tetime:
   begin
{$IFDEF DELPHI2009UP}
    FValidchars:=[FormatSettings.Timeseparator,'0'..'9'];
{$ELSE}
...
{$ENDIF}
  ...
  tefloat:
   begin
{$IFDEF DELPHI2009UP}
    FValidchars:=[FormatSettings.DecimalSeparator,'+','-','0'..'9','E','e'];
{$ELSE}
...
{$ENDIF}
  ...
  teCurrency:
   begin
{$IFDEF DELPHI2009UP}
    FValidchars:=[FormatSettings.DecimalSeparator,'+','-','0'..'9'];
{$ELSE}
...
begin
 // Mirem si es correcte el n�mero
  case FEditType of
   teinteger:
    strtoint(text);
   tefloat:
...
{$IFDEF DOTNETD}
 ...
 begin
  case FEditType of
   ...
   teFloat,teCurrency:
    Result:=(String(Key)=String(FormatSettings.DecimalSeparator));
  end;
 end;
{$ENDIF}
end;
...
begin
{$IFDEF DELPHI2009UP}
 if ((Message.CharCode=VK_DECIMAL) AND  (VK_DECIMAL<>Ord(FormatSettings.DecimalSeparator)) ) then
{$ELSE}
...
{$IFDEF DELPHI2009UP}
  PostMessage(self.handle,wM_CHAR,ord(FormatSettings.DecimalSeparator),0);
{$ELSE}
