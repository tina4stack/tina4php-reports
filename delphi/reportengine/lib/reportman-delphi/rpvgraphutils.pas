...
uses Classes,SysUtils,Windows,VCL.Graphics,rpmunits,VCL.Printers,WinSpool,
 rpmdconsts,rptypes,VCL.Forms,
{$IFDEF VCLNOTATION}
 Vcl.Imaging.jpeg,
{$ENDIF}
{$IFNDEF VCLNOTATION}
...
function GetPrintersOld: TStrings;
var
...
{$ENDIF}
  ...
  begin
   ...
   GlobalUnLock(DeviceMode);
   DeviceMode:=NDeviceMode;
   PDevMode := GlobalLock(NDeviceMode);
  end;
  PDevMode.dmFields:=dm_Orientation;
  if landscape then
   PDevMode.dmOrientation := 2
   ...
   PDevMode.dmOrientation := 1;
  DocumentProperties(0,nhan,Device, PDevMode^,
       PDevMode^, DM_MODIFY);
  ResetDC(nhan,PDevMode^);
 finally
  GlobalUnLock(DeviceMode);
 end;
end;
