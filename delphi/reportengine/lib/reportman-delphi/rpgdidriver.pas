...
uses
  mmsystem, windows,
  Classes, sysutils, rpmetafile, rpmdconsts, VCL.Graphics, VCL.Forms,
  rpmunits, VCL.Printers, VCL.Dialogs, VCL.Controls, rpgdifonts, Math,
  VCL.StdCtrls, VCL.ExtCtrls, rppdffile, rpgraphutilsvcl, WinSpool, rpmdcharttypes,
{$IFDEF FIREDAC}
  Firedac.VCLUI.Wait,
{$ENDIF}
{$IFDEF VCLNOTATION}
...
{$ENDIF}
{$ENDIF}
  rppdfdriver, rptextdriver, VCL.Mask, rpmaskedit;
  ...
  try
    try
     ...
     try
      ...
      bitmap.Assign(gpicture.Graphic);
      jpeg.Assign(bitmap); //AVZ fix for bad images like PNG messing up the jpeg!
