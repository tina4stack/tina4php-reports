...
uses SysUtils, Classes,Windows,
  ...
  VCL.Buttons, VCL.ExtCtrls, VCL.Controls, VCL.StdCtrls,
  rpmdconsts,rptypes,VCL.ComCtrls,rpmaskedit,VCL.checklst,VCL.Menus,
{$IFDEF USEVARIANTS}
  Variants,
{$ENDIF}
{$IFNDEF FORWEBAX}
...
begin
 ...
 // Check parameters
{$IFNDEF FORWEBAX}
 if (Trpbasereport(report).params<>nil) then
 begin
...
{$ENDIF}
   ...
   rpParamDateTime:
     begin
      ...
      acontrol.tag:=i;
{$IFDEF DELPHI2009UP}
      acontrol.Width:=Canvas.TextWidth(FormatDateTime(FormatSettings.ShortDateFormat,EncodeDate(2000,12,31)))+
   ...
   rpParamMultiple:
     begin
      ...
      begin
       ...
       TCheckListBox(acontrol).PopupMenu.Items.Add(TMenuItem.Create(acontrol));
       TCheckListBox(acontrol).PopupMenu.Items[0].Caption:=SRpClearSelection;
       TCheckListBox(acontrol).PopupMenu.Items[0].OnClick:=OnClearClick;
       TCheckListBox(acontrol).PopupMenu.Items[1].Caption:=SRpSelectAll;
  ...
  begin
   ...
   begin
    case fparams.Items[i].ParamType of
     ...
     rpParamTime:
      begin
       datevalue:=TDateTimePicker(LControls.Objects[i]).Date;
       datevalue:=Trunc(datevalue);
       fparams.items[i].Value:=datevalue;
      end;
     rpParamDateTime:
      begin
       datevalue:=Trunc(TDateTimePicker(LControls.Objects[i]).Date);
